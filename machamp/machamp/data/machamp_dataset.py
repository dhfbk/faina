...
class MachampDataset(Dataset):
    ...
    def set_label_distribution(self, dataset_configs):
        """
        Sets the label distribution for class weighting purposes in the following form:
        {"label_A": count, ...} on the dataset_config object only for the tasks for which
        class weighting has been specified.

        Parameters
        ----------
        dataset_configs: dict
            The dictionary to update with the new per-task key "class_weights_counts".
        """
        label_distribution = {}

        for task_name, task_config in self.dataset_config['tasks'].items():
            if 'class_weights' in task_config.keys():
                if task_config['class_weights'] != None:
                    label_distribution[task_name] = {}

                    for instance in self.data:
                        label = instance.golds[task_name]
                        if label not in label_distribution[task_name].keys():
                            label_distribution[task_name][label] = 1
                        else:
                            label_distribution[task_name][label] += 1

        for task_name in label_distribution.keys():
            self.dataset_config['tasks'][task_name]['class_weights_counts'] = label_distribution[task_name]
